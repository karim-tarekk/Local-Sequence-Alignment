...
def localAlign(x, y, localAlignment):

    # Align sequences globally
    matrix = getMatrix(x, y, localAlignment.gap)
    traceBack = traceback(x, y)
    # Adding match and mismatch values to diagonal and gap to up and left
    for i in range(1, len(x)+1):
        for j in range(1, len(y)+1):
            left = matrix[i][j-1] + localAlignment.gap
            up = matrix[i-1][j] + localAlignment.gap
            diag = matrix[i-1][j-1] + \
                localAlignment.MatchMismatch(x[i-1], y[j-1])
            # Adding 0 to the max() function in order to take it instead of any negative number (only if the negative number is the max)
...
def proteinLocalAlign(x, y, localAlignment):

    # Align proteins sequences locally using our functions
    matrix = getMatrix(x, y, localAlignment.gap)
    traceBack = traceback(x, y)
...
# Entering 1 for DNA Sequencing & 2 for Protein Sequencing
print("1-DNA Sequencing \n2-Protein Sequencing")
choice = input("Choice:")
...
if choice == "1":  # DNA Seqquencing
    ...
    print(y)
    print(" ")
    # Gap, Match and Mismatch values
    localAlignment = LocalAlignment(-1, 1, -2)
    matrix, traceBack = localAlign(x, y, localAlignment)
    ...
    # Gap, Match and Mismatch values
    localAlignment = LocalAlignment(-1, 1, -2)
    matrix, traceBack = proteinLocalAlign(x, y, localAlignment)
